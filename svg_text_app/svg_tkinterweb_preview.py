...
class SVGTextApp:
    def __init__(self, root):
        ...
        self.preview_frame = HtmlFrame(root, horizontal_scrollbar="auto", vertical_scrollbar="auto")
        # self.preview_frame.pack(side="right", fill="both", expand=True)
        self.preview_frame.grid(row=0, column=2, rowspan=6, sticky="nsew", padx=5, pady=5)
    ...
    def update_preview(self):
        ...
        svg_content = f"""<svg xmlns="http://www.w3.org/2000/svg" width="500" height="200">
            <text x="20" y="100" font-family="{font}" font-size="{size}" fill="{color}">{text}</text>
        </svg>"""

        with open("preview.svg", "w") as f:
            f.write(svg_content)

        self.preview_frame.load_file("preview.svg")  # Refresh preview
