...
function makeShortSvgs(csvFilePath) {
  ...
  rows.forEach((row, idx) => {
    ...
    const grad = `
      <defs>
        <linearGradient id="bg-gradient" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#8B5E3C" />  <!-- Brown tint -->
          <stop offset="100%" stop-color="#3C8B5E" /> <!-- Green tint -->
        </linearGradient>
      </defs>
    `;
    ...
    const descLines = getLanguageDesc(row.Language).map(
      (line, i) => `<text x="${WIDTH/2}" y="${470 + i*40}" text-anchor="middle" font-size="36" fill="#fff">${line}</text>`
    ).join('\n');
...
`;
  ...
  console.log('SVGs created for each language with gradient and descriptions.');
}
