...
function makeShortSvgs(csvFilePath) {
  ...
  rows.forEach((row, idx) => {
    ...
    // Title and logo
    const titlePath = svgTextAsCenteredPath(row.Language, font, 80, WIDTH/2, 180, "#fff");
    ...
    traitNames.forEach((trait, i) => {
      ...
      bars += 
        `<text x="${BAR_X-30}" y="${barY+BAR_HEIGHT-12}" font-size="44" fill="#222" text-anchor="end">${trait.replace('_', ' ')}</text>
        ...
        <text x="${BAR_X+BAR_WIDTH+20}" y="${barY+BAR_HEIGHT-12}" font-size="44" fill="#222" font-weight="bold">${score}</text>\n`;
    });

    // Description below bars, centered, bold
    const descLines = getLanguageDesc(row.Language).map(
      (line, i) => `<text x="${WIDTH/2}" y="${BAR_START_Y + traitNames.length*BAR_GAP + 60 + i*52}" text-anchor="middle" font-size="50" fill="#fff" font-family="Chivo-Bold, sans-serif" font-weight="bold">${line}</text>`
    ).join('\n');
...
`;
  ...
  console.log('SVGs created for each language with gradient and bold description below bars.');
}
