...
def fetch_formats():
    ...
    progress_var.set("Fetching formats...")
    ...
    try:
        ...
        for line in process.stdout:
            formats_text.insert(tk.END, line)
            formats_text.see(tk.END)
            root.update_idletasks()

        process.wait()
        if process.returncode == 0:
            progress_var.set("Formats fetched successfully.")
        else:
            progress_var.set("Failed to fetch formats.")
    except Exception as e:
        messagebox.showerror("Error", str(e))
        progress_var.set("Error fetching formats.")
